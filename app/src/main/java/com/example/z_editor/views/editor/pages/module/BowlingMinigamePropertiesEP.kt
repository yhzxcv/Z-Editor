...
@Composable
fun BowlingMinigamePropertiesEP(
    rtid: String,
    onBack: () -> Unit,
    ...
    Scaffold(
        ...
        if (showHelpDialog) {
            EditorHelpDialog(
                ...
                HelpSection(
                    title = "简要介绍",
                    body = "旧版沙滩保龄球的设置模块，作用仅为设置禁种线和禁用铲子，可用于常规关卡。"
                )
                HelpSection(
                    title = "禁种线",
                    body = "设置禁止种植植物的列数边界。数值越小，左侧可操作区域越小。"
                )
            }
        }
        Column(
            ...
            Card(
                ...
                Column(
                    ...
                    StepperControl(
                        label = "禁种线 (BowlingFoulLine)",
                        valueText = "${moduleDataState.value.bowlingFoulLine}",
                        onMinus = {
                            val current = moduleDataState.value.bowlingFoulLine
                            if (current > 0) {
                                moduleDataState.value =
                                    moduleDataState.value.copy(bowlingFoulLine = current - 1)
                                sync()
                            }
                        },
                        onPlus = {
