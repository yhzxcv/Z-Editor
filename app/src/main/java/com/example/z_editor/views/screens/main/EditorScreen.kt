    ...
    key(fileUri) {
        ...
        fun recalculateLevelState() {
            ...
            if (isVaseBreaker) {
                ...
                if (!existingClasses.contains("VaseBreakerArcadeModuleProperties")) missingList.add(
        ...
        LaunchedEffect(fileUri) {
            ...
            val file = LevelRepository.loadLevel(context, cacheName)
        ...
        fun performSave(isExit: Boolean = false) {
            val dataToSave = rootLevelFile ?: return
            val currentUri = fileUri ?: return
            ...
            EditorActions(
                ...
                onToggleSunDropperMode = { enableCustom, currentData ->
                    ...
                    if (enableCustom) {
                        ...
                        val existingObj =
                            objects.find { it.aliases?.contains(defaultAlias) == true }
                        if (existingObj == null) {
                            val newObj = PvzObject(
                    ...
                    val newObjectMap =
                        objects.associateBy { it.aliases?.firstOrNull() ?: "unknown" }
                    parsedData = parsedData!!.copy(
                        objectMap = newObjectMap,
                        levelDef = levelDef
                    )
